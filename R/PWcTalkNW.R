...
{
  write.csv(PWpair,paste0('PWpair.',figname,'.csv'),row.names=F,quote=F)
  write.csv(PWp,paste0('PWp.',figname,'.csv'),row.names=F,quote=F)
	if (max(PWpair[,3])<=1)
  ...
  names(PW.w) <- PWp[,1]
	...
	g <- graph_from_data_frame(PWpair,directed=F)
