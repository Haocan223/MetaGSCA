---
title: <center> <h2>Introduction to the MetaGSCA package</h2> </center>
...
<font size="3"> This vignette provides an overview of the Bioconductor package \Rpackage{MetaGSCA} for differential co-expression p-value. </font>    
...
<font size="3"> Create a temporary directory "workdir" under the user's current working directory. All output generated in this vignette will be saved in "workdir".  </font>  
...
The 2 sourse of data are necessary for running the examples and case studies in this vignette. </font>     
...
mygeneset <- list.files(system.file("data", package = "MetaGSCA"),
...
genesetnames <- gsub(paste(system.file("data", package = "MetaGSCA"),
                     "/", sep = ""), "", genesetnames)

##########################
## step 2,read in datasets
mydataset <- c(paste(system.file("data", package = "MetaGSCA"),
                     "/KIRP.csv", sep=""),
               paste(system.file("data", package = "MetaGSCA"),
                     "/LIHC.csv", sep=""))
datasets <- list(); groups <- list(); datasetnames <- list()
for (j in 1:length(mydataset)) {
    d <- fread(mydataset[j], header = TRUE, data.table = FALSE)
    datasets[[j]] <- d[-1,]
    groups[[j]] <- as.character(d[1, -1])
    datasetnames[[j]] <- tools::file_path_sans_ext(mydataset[j])
}
datasetnames <- gsub(paste(system.file("data", package = "MetaGSCA"),
                     "/", sep = ""), "", datasetnames)
```
...
**2. lists of datasets (KIRP & LIHC):**   
*  **KIRP** (17759 rows & 65 columns) contain 17758 genes & 32 individuals, individuals are equally distributed in 2 groups (group1 & group2).    
*  **LIHC** (16796 rows & 101 columns) contain 16795 genes & 50 individuals, individuals are equally distributed in 2 groups (group1 & group2).    
</font>   
<br>
...
result <- paste(system.file("data", package = "MetaGSCA"), "/single.png", sep="")
...
result1 <- paste(system.file("data", package = "MetaGSCA"), "/Aminosugars metabolism_GLMM_Meta Analysis for bootstrap p-value_Forest plot.png", sep="")
...
```{r echo=FALSE}
result2 <- paste(system.file("data", package = "MetaGSCA"), "/Aminosugars metabolism_Individual Dataset GSAR Results with Bootstrapping.csv", sep="")
t(read.csv(result2))
```
<br>
...
<br>
```{r echo=FALSE, out.width = "60%", fig.align='left', dpi=200}
result <- paste(system.file("data", package = "MetaGSCA"), "/multiple.png", sep="")
knitr::include_graphics(result)
```
<br>
...
result1 <- paste(system.file("data", package = "MetaGSCA"), "/Arginine and Proline metabolism_GLMM_Meta Analysis for bootstrap p-value_Forest plot.png", sep="")
knitr::include_graphics(result1)
```
<br>
...
```{r echo=FALSE}
result2 <- paste(system.file("data", package = "MetaGSCA"), "/Arginine and Proline metabolism_Individual Dataset GSAR Results with Bootstrapping.csv", sep="")
t(read.csv(result2))
```
<br>
...
warn <- paste(system.file("data", package = "MetaGSCA"), "/GLMM warning.png", sep="")
...
```{r warning=FALSE, eval=FALSE}
## step 4, call PWcTalk function (4.1) or, alternatively, a code block (4.2)to perform pathway crosstalk analysis
#### NOTE: One must execute MetaGSCA_Multi_Geneset() on sufficiently many gene sets (say >100) before attempting step 4.
#### To enable swift testing of pathway cross talk analysis, we provide two existent example input files as if they were generated from step 3.2.
dir.create(file.path(workdir, 'PWcTalk'), showWarnings = FALSE)
setwd(file.path(workdir, "PWcTalk"))
## step 4.1, one code line to execute pathway crosstalk analysis. Simple, but no flexibility for layout tuning.
PWcTalk(infile=file.path(workdir,'PWcTalk/input2PWcTalk.real.csv'),test='binary',
...
infile <- file.path(workdir,'PWcTalk/input2PWcTalk.small.csv')
pTh.dataset=0.01
pTh.pwPair=0.01
pTh.pw=0.01
test='phi'
dichotGSAR.res <- dichotGSAR(infile,pTh.dataset=pTh.dataset)
PWsim.res <- PWsim(dichotGSAR.res$dichotP,test=test)
...
g_tkid <- PWcTalkNW(PWpair,PWp)
##### PAUSE: user can adjust the network layout on the pop-out graphic window identified with an integer of g_tkid$tkid.
coords <- tk_coords(g_tkid$tkid)
g_tkid <- PWcTalkNW(PWpair,PWp,layout=coords,pdfW=14,pdfH=10,figname='PWcTalk',asp=0.5)
```
<br>
